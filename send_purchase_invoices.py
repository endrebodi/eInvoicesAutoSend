...
if response.status_code == 200:
    ...
    conn = sqlite3.connect('purchase_invoices.db')
    cursor = conn.cursor()
    ...
    for id in purchase_invoice_ids:
        if id not in existing_ids:
            ...
            invoice_url = f"https://efaktura.mfin.gov.rs/api/publicApi/purchase-invoice/xml?invoiceId={id}"
            ...
            if invoice_response.status_code == 200:
                ...
                message['Subject'] = f'Ulazna_faktura_{id}'
                message['From'] = 'SENDERNAME'
                message['To'] = 'RECEIVEREMAILADDRESS'
                xml_attachment = MIMEApplication(xml_data, _subtype='xml')
                xml_attachment.add_header('Content-Disposition', 'attachment', filename=f'Ulazna_faktura_{id}.xml')
                message.attach(xml_attachment)
                pdf_attachment = MIMEApplication(pdf_data, _subtype='pdf')
                pdf_attachment.add_header('Content-Disposition', 'attachment', filename=f'Ulazna_faktura_{id}.pdf')
